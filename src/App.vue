...
onMounted(() => {
  try {
    ...
    // If there's an error, start with an empty notes array
    notes.value = [];
  }
});
...
<template>
  <main class="min-h-screen w-screen bg-gray-100">
    ...
    <div class="w-full max-w-6xl mx-auto p-4 sm:p-6 lg:p-8">
      <header
        class="flex flex-col sm:flex-row justify-between items-center gap-4 sm:gap-0 p-4 sm:p-6 lg:p-8"
      >
        <h1
          class="text-4xl sm:text-5xl lg:text-7xl font-bold text-gray-800 text-center sm:text-left"
        >
          Notes
        </h1>
        <button
      ...
      <div
        class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mt-6"
      >
        <div
          v-for="note in notes"
          :key="note.id"
        ...
        >
          ...
          <div v-else class="flex flex-col h-full">
            <p class="main-text text-sm flex-grow leading-relaxed text-white/90">
              {{ note.text }}
            </p>
            <div class="flex justify-between items-center mt-auto pt-2">
              ...
              <div
                class="opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-200 flex gap-1"
              >
                <button
